...
const getGlobalGlossaryTerms = async (
  id: string,
): Promise<GlobalGlossaryTerms> => {
  try {
    ...
    console.warn(
      `Failed to read/verify cached global glossary terms for ${id}, fetching fresh.`,
      e,
    );
  }
  const fresh = await client
    .get(`global-glossary/${id}/terms`)
    .json<GlobalGlossaryTerms>();
  try {
    await GlobalGlossaryCacheRepo.set(fresh);
  } catch (e) {
    console.warn(
      `Failed to save fresh global glossary terms to cache for ${id}`,
      e,
    );
  }
  return fresh;
